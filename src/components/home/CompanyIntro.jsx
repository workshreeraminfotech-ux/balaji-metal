    ...
    <section className="py-10 sm:py-12 lg:py-14 bg-white text-slate-900 border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-stretch">
          ...
          {/* Visual Showcase (6 cols) - Matches exact height of content */}
          <motion.div 
            className="lg:col-span-6 relative flex flex-col order-2 lg:order-1"
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.6, ease: "easeOut" }}
          >
            <div className="relative rounded-3xl overflow-hidden border border-slate-200 shadow-lg shadow-slate-200/60 bg-slate-100 group flex-1 min-h-[350px] sm:min-h-[400px]">
              <img 
          ...
          {/* Text Content (6 cols) */}
          <motion.div 
            className="lg:col-span-6 flex flex-col justify-between space-y-5 order-1 lg:order-2"
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.6, ease: "easeOut" }}
          >
            ...
            <div className="pt-2">
              <Link to="/about">
                <Button className="bg-slate-900 hover:bg-slate-800 text-white font-bold px-6 py-3 rounded-xl flex items-center gap-2 text-sm group shadow-md shadow-slate-900/10 cursor-pointer">
